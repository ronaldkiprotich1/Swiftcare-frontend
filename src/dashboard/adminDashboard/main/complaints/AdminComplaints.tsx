...
import { Edit, Trash2, MessageSquare, User, Calendar, CheckCircle, XCircle, Clock, AlertCircle } from "lucide-react";
import { useState } from "react";
import UpdateComplaint from "./UpdateComplaint";
import DeleteComplaint from "./DeleteComplaints";
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-gray-600 mt-1">
                            Manage patient complaints – {complaintsData.length} total complaints
                        </p>
                    </div>
                </div>
            </div>
            ...
            {complaintsData.length > 0 ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {complaintsData.map((complaint) => (
                        <div key={complaint.complaintId} className="bg-white rounded-xl shadow-md border overflow-hidden hover:shadow-lg transition">
                            {/* Header */}
                            <div className="bg-gradient-to-r from-teal-50 to-pink-50 p-4 border-b">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <h3 className="text-lg font-bold text-gray-900">Complaint #{complaint.complaintId}</h3>
                            ...
                            <div className="p-6">
                                <div className="mb-4">
                                    ...
                                    <p className="font-medium bg-gray-50 rounded-lg p-3">{complaint.subject}</p>
                                </div>

                                <div className="mb-4">
                                    <h4 className="text-sm font-semibold mb-2">Description</h4>
                                    <p className="text-gray-700 bg-gray-50 rounded-lg p-3 text-sm">{complaint.description}</p>
                                </div>
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="bg-teal-50 rounded-lg p-3 text-sm">
                                        <p><span className="font-medium">User ID:</span> {complaint.userId}</p>
                                        {complaint.relatedAppointmentId && (
                                            <p className="mt-1">
                                                <span className="font-medium">Related Appointment:</span> #{complaint.relatedAppointmentId}
                                            </p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => handleChangeStatus(complaint)} className="flex-1 btn btn-outline border-blue-200 text-blue-700">
                                        <Clock className="h-4 w-4" /> Status
                                    </button>
                                    <button onClick={() => handleEdit(complaint)} className="flex-1 btn btn-outline border-teal-200 text-teal-700">
                                        <Edit className="h-4 w-4" /> Edit
                                    </button>
                                    <button onClick={() => handleDelete(complaint)} className="flex-1 btn btn-outline border-red-200 text-red-700">
                ...
                <div className="bg-white rounded-lg shadow-sm border p-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="text-center p-4 bg-teal-50 rounded-lg">
                            <div className="text-2xl font-bold text-teal-600">{complaintsData.length}</div>
